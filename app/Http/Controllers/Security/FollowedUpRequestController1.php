...
use App\Http\Requests\Security\FollowedUpRequestRequest11;
...
class FollowedUpRequestController extends Controller
{
    ...
    public function index() {
        return view('pages.security.followed_up_request.list');
    }

    public function show($id) {
        $item = UserRequest::findOrFail($id);

        return view('pages.security.followed_up_request.show', [
            'item'  => $item 
        ]);
    }
    ...
    public function edit($id) {
        ...
        $securitys    = User::where('role', 'security')->get();

        return view('pages.security.followed_up_request.edit', [
            'item'          => $item,
            'securitys'    => $securitys
        ]);
    }

    public function update(FollowedUpRequestRequest1 $request, $id) {
        $data = $request->all();
        ...
        return redirect()->route('security.f-up-request');
    }
}
